...
Data heterogeneity (Non-IID) poses severe challenges in One-shot FL (OFL) due to the lack of iterative correction~\cite{amato2025survey}. While multi-round methods like FedProx~\cite{li2020} and SCAFFOLD~\cite{karimireddy2020} address this via frequent communication, OFL requires robust single-round solutions. Existing approaches largely fall into three categories: (1) \textbf{Distillation-based} methods like DENSE~\cite{zhang2022} and \textbf{Co-Boosting~\cite{dai2024coboosting}} simulate global data interactions. \textbf{While Co-Boosting attempts to mitigate generator quality issues via adversarial hard-sample mining, our experiments show it still suffers from generator mode collapse under extreme skew (achieving only 19.24\% on CIFAR-100), as the disjoint ensemble cannot guide the generator effectively;} (2) \textbf{Aggregation-based} methods like FedLPA~\cite{liu2024} employ advanced Bayesian inference to weight parameters; however, as shown in our experiments, they struggle in one-shot settings with deep networks where permutation symmetries prevent substantial parameter alignment; and (3) \textbf{Client-side} enhancements like FAFI~\cite{zeng2025} focus on local feature quality. Recent work \textbf{AFL}~\cite{AFL_He_CVPR2025} proposes an analytic approach for one-shot FL. However, AFL strictly relies on pre-trained frozen backbones to extract embeddings and solves a linear regression problem. As shown in our experiments, when applied to the standard FL setting (training from scratch) without pre-trained weights, AFL fails to learn discriminative features (20.10\%), whereas AURORA successfully learns high-quality representations (48.83\%) autonomously.

\textbf{Progressive and Split Approaches:} Recent works like FuseFL~\cite{tang2024fusefl} attempt to break the isolation of local training by progressively fusing model blocks in a bottom-up manner. FuseFL views local training through a causal lens, arguing that feature augmentation from other clients removes spurious correlations. However, FuseFL strictly requires $K$ sequential communication rounds (where $K$ is the number of blocks), violating the strict single-interaction constraint of OFL if $K>1$. Furthermore, our experiments show that without a global coordinate system, progressive fusion struggles to align features under extreme label skew (CIFAR-100, $\alpha=0.05$), achieving only 32.71\% compared to AURORA's 48.83\%.

Unlike methods relying on complex auxiliary transmission (e.g., FALCON~\cite{liu2026feature}) or server-side generation, AURORA purely regulates \emph{local training dynamics} via geometric anchors to produce alignable models without extra communication overhead. Since AURORA relies on fixed ETF anchors, it also avoids prototype poisoning risks associated with dynamic prototype learning~\cite{zeng2025bapfl}.
...
Crucially, $\Lalign$ is computed solely between learnable prototypes and fixed anchors. \textbf{Gradient descent on $\Lalign$ updates the prototypes directly, but does not backpropagate to the backbone encoder.} The encoder aligns to the global geometry indirectly, as $\Llocal$ pulls features towards these ETF-aligned prototypes.

\textbf{ETF Construction Requirement.} The simplex ETF requires embedding dimension $d \geq C-1$. With ResNet-18 ($d=512$), this is satisfied for CIFAR-10 ($C=10$) and CIFAR-100 ($C=100$). For high-cardinality tasks where $C>d$, we introduce a Projection Head $g(\cdot)$ to map features $z=g(f(x))$ into a higher-dimensional space $\mathbb{R}^{d'}$ where $d' \ge C-1$, calculating $\Lalign$ on $z$. This ensures geometric feasibility for any number of classes (empirically validated in Section 4.7).
...
\textbf{Co-Boosting} & Distil./Ensemble & 19.24 $\pm$ 1.42 & -29.59\% \\
\textbf{AFL (Scratch)} & Analytic & \textbf{20.10} & -28.73\% \\
\textbf{One-Shot FedETF} & Static Alignment & 23.90 (Ensemble) & -24.93\% \\
\textbf{IntactOFL} & Mixture-of-Experts & 27.99 $\pm$ 0.67 & -20.84\% \\
\textbf{FuseFL ($K=2$)} & Progressive Fusion & 29.98 & -18.85\% \\
\textbf{FuseFL ($K=4$)} & Progressive Fusion & 32.71 & -16.12\% \\
...
\end{small}
\end{center}
\footnotesize{Note: For fair comparison, we evaluate AFL without pre-trained weights (training from scratch), consistent with other baselines.}
\end{table*}
...
\textbf{Limitations of Advanced Distillation (Co-Boosting):} We also compared AURORA against the recent \textbf{Co-Boosting} framework~\cite{dai2024coboosting}, which attempts to enhance One-Shot FL by mutually boosting synthetic data quality and ensemble weights. As shown in Table~\ref{tab:main-results-c100}, while Co-Boosting (19.24\%) performs comparably to DENSE (20.52\%) and outperforms FedAvg, it significantly trails AURORA (48.83\%) by nearly \textbf{30\%}. This result validates our core observation regarding extreme heterogeneity ($\alpha=0.05$):
\begin{itemize}
    \item \textbf{Generator Failure:} Co-Boosting relies on generating `hard samples' from the ensemble to refine the server model. However, in extreme non-IID settings (CIFAR-100, $\alpha=0.05$), the ensemble is derived from highly disjoint local models. The resulting generator suffers from severe mode collapse, failing to synthesize representative data regardless of the adversarial `boosting' mechanism.
    \item \textbf{Feature vs. Generation:} This confirms that trying to \emph{generate} missing data (Co-Boosting/DENSE) is algorithmically brittle compared to AURORA's approach of \emph{aligning} feature spaces via geometric anchors. AURORA succeeds because it solves the alignment problem directly in the feature space, rather than relying on a generator that cannot learn the global distribution from sparse, disjoint clients.
\end{itemize}

We also compare against the recent IntactOFL~\cite{zeng2024one}, which attempts to mitigate model inconsistency by maintaining local models as a Mixture-of-Experts (MoE). As shown in Table~\ref{tab:main-results-c100}, IntactOFL achieves 27.99\%, outperforming FedAvg (15.59\%) and DENSE (20.52\%). This confirms that avoiding destructive parameter merging is beneficial. However, AURORA (48.83\%) still outperforms IntactOFL by a massive \textbf{20.84\%} margin. This gap highlights a critical finding: \textbf{In extreme non-IID settings, ``better aggregation'' (like MoE) is insufficient if the local models are fundamentally misaligned or biased.} IntactOFL freezes these biased local experts, effectively locking in their poor generalization. In contrast, AURORA's dynamic regularization forces the local models to learn alignable features \emph{during} training, treating the root cause of the heterogeneity rather than just managing the symptoms at aggregation.

\textbf{Limitations of Progressive Fusion (FuseFL):} We further compared AURORA against FuseFL~\cite{tang2024fusefl}, a recent causal-based approach that splits the model into $K$ blocks and performs progressive bottom-up fusion to augment features. While FuseFL ($32.71\%$ with $K=4$) outperforms IntactOFL ($27.99\%$) and FedAvg ($15.59\%$) by mitigating spurious correlations via feature swapping, it still trails AURORA ($48.83\%$) by over $16\%$. This gap highlights a critical limitation of bottom-up fusion in extreme non-IID settings ($\alpha=0.05$): when local data shards are too small and biased, the early-stage blocks ($k=1,2$) learned by FuseFL are already fundamentally misaligned. Progressively fusing these ``poisoned'' foundations cannot recover the global semantic structure. In contrast, AURORA provides \textbf{top-down} geometric guidance (via the fixed Simplex ETF) that anchors the feature space \emph{before} the model drifts, ensuring that local features remain alignable regardless of data sparsity.

\textbf{The Key Insight: Feature Space vs. Parameter Space.} The dramatic gap between FedAvg (15.59\%) and AURORA (48.83\%) underscores a fundamental principle: in one-shot non-IID settings, \emph{parameter-space} alignment is algorithmically brittle due to permutation symmetries. AURORA succeeds because it enforces alignment in the \emph{feature space} via explicit ETF geometric anchors, which are invariant to internal parameter permutations.
...
We analyze AURORA's learned $\lambda$ trajectories compared to manual annealing. Table~\ref{tab:lambda-evolution} shows that AURORA discovers a comparable schedule to manual tuning, and Figure~\ref{fig:lambda-divergence-appendix} reveals that clients develop \emph{divergent} trajectories despite sharing the same $s(p)$ prior---demonstrating AURORA is data-dependent, not merely time-dependent. Extended analysis is provided in Appendix~\ref{appendix:schedule-analysis}.
...
\subsection{Scalability to Large-Scale \& High-Cardinality Tasks}

To address concerns regarding scalability and dimensionality constraints (where $C \ge d$), we evaluated AURORA on \textbf{Tiny-ImageNet} (200 classes). As shown in Table~\ref{tab:tiny-imagenet}, utilizing a Projector to map features to a higher-dimensional space ($512 \to 2048$) boosts accuracy from 42.78\% to \textbf{47.35\%}. Notably, even with simple parameter averaging (FedAvg), AURORA achieves 40.35\%, demonstrating robust alignment capabilities in complex visual domains where standard One-shot methods typically collapse.

\begin{table}[h]
\caption{Scalability on Tiny-ImageNet (200 Classes). A Projector effectively resolves the dimensionality bottleneck, achieving SOTA performance.}
\label{tab:tiny-imagenet}
\begin{center}
\resizebox{\columnwidth}{!}{
\begin{tabular}{llccl}
\toprule
\textbf{Dataset} & \textbf{Method} & \textbf{Feat. Dim} & \textbf{Agg} & \textbf{Acc} \\
\midrule
Tiny-ImageNet & FAFI (Baseline) & 512 & IFFI & 42.78\% \\
(200 Classes) & AURORA (No Proj) & 512 & IFFI & 43.96\% \\
 & \textbf{AURORA (Proj)} & \textbf{2048} & \textbf{IFFI} & \textbf{47.35\%} \\
